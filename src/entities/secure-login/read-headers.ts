...
class LoadJWT {
  ...
  public decode(): PHP.decodedToken {
    ...
    if (tokens.length === 0) {
      ...
      return tokens[0] !== undefined ? tokens[0] : this.decodeTemplate();
    }
  }
   ...
   */
  public decodeTemplate(): PHP.decodedToken {
    return {
      exp: 0,
      aud: "",
   ...
   * Gives an decoded token object with all fields sette to nulled and frozen.
   */
  public static decodeTemplate(): PHP.decodedToken {
    return {
      exp: 0,
      aud: "",
      data: Object.freeze({ iss: "", uid: "", usr: "", aut: "none", rus: "" }),
   ...
   */
  public encodeTemplate(): PHP.encodedToken {
    return {
      data: "",
    };
  }
   ...
   * Gives an encoded token object with all fields sette to nulled and frozen.
   */
  public static encodeTemplate(): PHP.encodedToken {
    return {
      data: "",
    };
  }
      ...
      try {
        ...
        if (
          !checkObjectValidKeys(
            nameAndType(LoadJWT.encodeTemplate()),
            encodedToken
          )
        ) {
          return "";
